...
public interface ExternalParameterMapper {
    @Insert("insert into jm_external_parameter(id, ref, name, type, value, label, association_id, association_type, association_platform, created_time, last_modified_time) " +
            "values(#{id}, #{ref}, #{name}, #{type}, #{value}, #{label},#{associationId}, #{associationType}, #{associationPlatform}, #{createdTime}, #{lastModifiedTime})")
    void add(ExternalParameter externalParameter);

    @Delete("delete from jm_external_parameter where id = #{id}")
    void deleteById(String id);

    @Delete("DELETE FROM `jm_external_parameter` WHERE `association_id` = #{associationId} AND `association_type` = #{associationType} AND association_platform = #{associationPlatform}")
