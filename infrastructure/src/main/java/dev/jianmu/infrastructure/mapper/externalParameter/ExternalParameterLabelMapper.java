...
public interface ExternalParameterLabelMapper {
    @Insert("insert into jm_external_parameter_label(id, value) " +
            "values(#{id}, #{value})")
    void add(ExternalParameterLabel externalParameterLabel);


    @Select("SELECT * FROM `jm_external_parameter_label`")
    @Result(column = "id", property = "id")
    @Result(column = "value", property = "value")
    List<ExternalParameterLabel> findAll();

    @Delete("delete from jm_external_parameter_label where id = #{id}")
    void deleteById(String id);

    @Update("update jm_external_parameter_label set value = #{value} where id = #{id}")
    void updateById(ExternalParameterLabel externalParameterLabel);

    @Select("SELECT * FROM `jm_external_parameter_label` WHERE id = #{id}")
    @Result(column = "id", property = "id")
    @Result(column = "value", property = "value")
    Optional<ExternalParameterLabel> findById(String id);

    @Select("SELECT * FROM `jm_external_parameter_label` WHERE value = #{value}")
    @Result(column = "id", property = "id")
    @Result(column = "value", property = "value")
