...
public interface ProjectGroupMapper {
    @Insert("insert into project_group(id, name, description, sort, is_show, project_count, created_time, last_modified_time) " +
            "values(#{id}, #{name}, #{description}, #{sort}, #{isShow}, #{projectCount}, #{createdTime}, #{lastModifiedTime})")
    void add(ProjectGroup projectGroup);

    @Insert("<script>" +
            "INSERT INTO `project_group`(`id`, `name`, `description`, `sort`, `is_show`, `project_count`, `created_time`, `last_modified_time`) VALUES" +
    ...
    @Update("update project_group set name=#{name}, description=#{description}, is_show=#{isShow}, last_modified_time=#{lastModifiedTime} where id=#{id}")
