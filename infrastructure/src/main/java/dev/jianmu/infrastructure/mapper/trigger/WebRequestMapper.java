...
public interface WebRequestMapper {
    @Insert("insert into jm_web_request(id, project_id, workflow_ref, workflow_version, trigger_id, user_agent, status_code, error_msg, request_time) " +
    ...
    @Select("SELECT * FROM jm_web_request where project_id = #{projectId} order by request_time desc")
    ...
    @Select("SELECT * FROM jm_web_request where id = #{id}")
    @Result(column = "project_id", property = "projectId")
    @Result(column = "workflow_ref", property = "workflowRef")
    @Result(column = "workflow_version", property = "workflowVersion")
    @Result(column = "trigger_id", property = "triggerId")
