...
public interface NodeDefinitionMapper {
    @Select("SELECT * FROM jm_hub_node_definition WHERE id = #{id}")
    ...
    @Select("<script>" +
            "SELECT * FROM `jm_hub_node_definition` " +
            "<where>" +
    ...
    @Insert("insert into jm_hub_node_definition(id, icon, name, owner_name, owner_type, owner_ref, creator_name, creator_ref, type, description, ref, source_link, document_link, deprecated) " +
    ...
    @Delete("delete from jm_hub_node_definition where id = #{id}")
