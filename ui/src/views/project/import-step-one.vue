      ...
      <div v-if="authe" class="authentication">
        <div>
          <jm-form-item label="类型" prop="credential.type">
            <jm-select
              v-model="gitCloneForm.credential.type"
              clearable
              disabled
              placeholder="请选择类型"
          ...
          <template
            v-if="
              gitCloneForm.credential.type === ProjectImporterTypeEnum.HTTPS
            "
          >
            ...
            <jm-form-item label="密码" prop="credential.passKey">
              <jm-select
                v-model="gitCloneForm.credential.passKey"
                clearable
                placeholder="请选择密码"
                no-data-text="请选择用户名"
              >
          ...
          <jm-form-item
            v-else-if="
              gitCloneForm.credential.type === ProjectImporterTypeEnum.SSH
            "
            label="私钥"
  ...
  setup() {
    ...
    const gitCloneFormRule = ref<object>({
      ...
      credential: {
        ...
        passKey: [{ required: true, message: '请选择密码', trigger: 'change' }],
      },
    });
    const passKeys = ref<string[]>([]);
    const loading = ref<boolean>(false);
    const authe = ref<boolean>(true);
    const nextStep = inject('nextStep') as (
      git: IGitVo,
    ...
    return {
      ...
      handleUserKeyChange: async (newVal: any) => {
        if (newVal) {
          ...
          passKeys.value = await listSecretKey(
            gitCloneForm.value.credential.namespace,
          );
        } else {
          delete gitCloneForm.value.credential.namespace;
          delete gitCloneForm.value.credential.userKey;
