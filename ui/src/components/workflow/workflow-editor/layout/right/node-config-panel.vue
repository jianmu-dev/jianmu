    ...
    <div class="jm-workflow-editor-node-config-panel">
      <jm-scrollbar v-if="drawerOpening" class="panel-container">
        ...
        <template v-else-if="nodeData.getType() === NodeTypeEnum.WEBHOOK">
          <custom-webhook-panel v-if="isCustomWebhook" :node-data="nodeData" @form-created="handleFormCreated" />
          <webhook-panel v-else :node-data="nodeData" @form-created="handleFormCreated" />
        </template>
        <shell-panel
